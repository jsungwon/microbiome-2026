...
논문 7편의 공개 데이터(ENA/SRA)를 내려받아 앰플리콘 파이프라인(DADA2)으로 처리하고,
**논문 보충자료의 샘플별 임상정보와 결합**해 둔 수업용 자료다.
FASTQ를 다루지 않고 TSV만 읽어서 바로 분석할 수 있다.

> 원 데이터는 각 논문 저자의 것이며 이 저장소는 수업 목적의 재가공물이다.
> 결과를 발표할 때는 **원 논문 7편을 인용**해야 한다 → [`docs/data_provenance.md`](docs/data_provenance.md)
...
| `07_skincare` | 화장품 중재 | MicrobiologyOpen 2021 | 0주 / 2주 / 4주 (25명 종단) | 16S 75 |
